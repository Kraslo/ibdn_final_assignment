...
CONNECTION_STRING = 'mongodb://mongotainer:27017/'
...
producer = KafkaProducer(bootstrap_servers=['kafkaibdn:9092'],api_version=(0,10))
...
def kafka_listener():
  ...
  def kafka_setup():
    from kafka import KafkaConsumer
    consumer = KafkaConsumer('flight_predictions',
        bootstrap_servers=['kafkaibdn:9092'],
        auto_offset_reset='earliest',
        enable_auto_commit=True,
        group_id='kafkasocket',
        heartbeat_interval_ms=3000,
        session_timeout_ms=20000,
        max_poll_records=1)
    logging.warning('test2')
    #consumer = KafkaConsumer('flight_prediction',
  #			   bootstrap_servers=['kafkaibdn:9092'],
  #			   enable_auto_commit=True,
  #			   group_id='kafkasocket')
    while True:
      warnings.warn('CONSUMER')
      logging.warning('infor')
      for message in consumer:
        response = json.loads(message.value.decode('utf-8'))
        #warnings.warn(response)
        logging.info('INFO RESPONSE INCOMING')
        logging.warning(response)
        jsonresponse = json.dumps(response).encode()
        socketio.emit('prediction_response', json.dumps(response))
  try:
    kafka_setup()
  except Exception:
    logging.warning('NO SE HA ENCONTRADO UN BROKER. INTENTANDO DE NUEVO.')
    time.sleep(2)
    kafka_listener()
...
# APARTADO 2 AÑADIMOS HANDLERS DE SOCKETIO
# @socketio.on
...
# COMENTADO POR AHORA, MIRAR DESPUÉS PARA QUÉ SIRVE!!!! # check chek
...
def classify_flight_delays_realtime():
  ...
  warnings.warn(str(prediction_features))
